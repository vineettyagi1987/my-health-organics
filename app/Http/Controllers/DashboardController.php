...
public function updateProfile(Request $request)
{
    ...
    // ✅ same normalization as registration
   $data['phone'] = preg_replace('/\D/', '', $data['phone']); // remove non-digits
$data['phone'] = ltrim($data['phone'], '0'); // remove leading 0
     $request->merge($data);
   $request->validate([
    ...
    'profile_photo' => 'nullable|image|mimes:jpg,jpeg,png|max:5120', // max 5MB
]);
    ...
    $idCard = $user->id_card;
      ...
      if (request()->hasFile('profile_photo')) {
        ...
        $canvas = $manager->create($width, $height)->fill('#f8fafc');
        // shadow circle
        $shadow = $manager->create(1000, 1000)->fill('rgba(0,0,0,0.1)');
        $canvas->place($shadow, 'center', 0, -75);
        // ================================
        // 🌍 Top Title
        // ================================
